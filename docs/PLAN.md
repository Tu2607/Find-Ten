...
- Action result channels remain error-only acknowledgements; snapshots are not returned through action results.
- Step 14 is the target architecture for timer/session runtime behavior, and Step 20 extends it by generalizing move requests into player action requests.
